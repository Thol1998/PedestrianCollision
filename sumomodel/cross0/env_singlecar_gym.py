...
class SUMO_ENV(gym.Env):
    ...
    def __init__(self,nogui=True):
        ...
        self.Observe_Position_x1 = 80
        self.Observe_Position_x2 = -40
        # 记录当前交叉口车辆状态
    ...
    def _get_state(self):
        ...
        for person_id, _ in person_list[0:self.MAX_PERSON_COUNT]:
            ...
            # states[6 + person_count * 4] = self.distance_vehicle_pedestrians(veh_id,person_id)
            states[7 + person_count * 4] = traci.person.getPosition(person_id)[1]
            # states[7 + person_count * 4] =self.distance_vehicle_pedestrians(veh_id,person_id)
    ...
    def get_total_reward(self):
        ...
        total_reward= speed_reward + pedestrian_reward+stop_num_reward+stop_num_reward
    ...
    def is_brake(self,veh_id):
        veh_x2,veh_y=traci.vehicle.getPosition(veh_id)
        veh_v=traci.vehicle.getSpeed(veh_id)
        veh_x1=veh_x2+veh_v*self.actionStepLength
        veh_right = veh_y + traci.vehicle.getWidth(veh_id) * 0.5
        veh_left = veh_y - traci.vehicle.getWidth(veh_id) * 0.5
        for ped_id in self.curr_vehicle_pedestrian:
            ped_x, ped_y2 = traci.person.getPosition(ped_id)
            ped_v = traci.person.getSpeed(ped_id)
            if veh_x2<=ped_x and veh_x1>=ped_x:
                if int(re.findall('\d', ped_id)[0]) == 1:
                    ped_y1 = ped_y2 + ped_v * self.actionStepLength
                    if (ped_y2<=veh_right and ped_y1>=veh_right) or (ped_y1<=veh_right and ped_y1>=(veh_left+ped_v*self.actionStepLength)):
                        print(self.curr_vehicle_id, 'broken! ')
                        self.curr_broken = True
                        return True
                else:
                    ped_y1 = ped_y2 - ped_v * self.actionStepLength
                    if (ped_y2 >= veh_left and ped_y1 <= veh_left) or (ped_y1>=veh_left and ped_y1<=(veh_right-ped_v*self.actionStepLength)):
                        print(self.curr_vehicle_id, 'broken! ')
                        self.curr_broken = True
                        return True
        self.curr_broken = False
    ...
    def del_vehicle(self, veh_id):
        traci.vehicle.moveToXY(veh_id, 'gneE_3', 0, x=-97, y=0)
    ...
    def veh_is_observe(self,veh_id):
        ...
        if (traci.vehicle.getPosition(veh_id)[0] <= self.Observe_Position_x1) \
                and (traci.vehicle.getPosition(veh_id)[0] >= self.Observe_Position_x2):
            return True
        return False
