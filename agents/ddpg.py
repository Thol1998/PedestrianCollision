...
class Agent(object):
   ...
   def select_action(self, obs):
      ...
      if self.curr_episode<=episode_number:
         action += ((episode_number-self.curr_episode) / episode_number) * self.act_noise * np.random.randn(self.act_dim)
   ...
   def train_model(self):
      ...
      policy_loss = -self.qf(obs1, self.policy(obs1)).mean()
   ...
   def run(self, max_step):
      ...
      veh_id, obs, reward, next_obs, done = self.env.reset()
      actions_set=[]
      ...
      while not (done or step_number == max_step):
         if self.eval_mode:
            ...
            veh_id, obs, reward, next_obs, done = self.env.step(action)
         else:
            self.steps += 1
         ...
         actions_set.append(action[0])
         total_reward += reward
         step_number += 1
         obs = next_obs
      if not self.eval_mode:
          print("veh_id:",veh_id,'total reward:',total_reward,' len:',len(actions_set),'elements:',actions_set)
      ...
      return step_number, total_reward,self.env.curr_broken
