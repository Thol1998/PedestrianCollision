...
parser.add_argument('--threshold_return', type=int, default=20,
...
def main():
    ...
    env = ENV()
    obs_dim = 12#env.observation_space
    act_num = 3#env.action_space
    ...
    train_episode_return_100=0
    for episode in range(1, args.training_eps+1):
        # Perform the training phase, during which the agent learns
        agent.eval_mode = False

        # Run one episode
        train_step_length, train_episode_return = agent.run(args.max_step,writer)
        
        train_num_steps += train_step_length
        train_sum_returns += train_episode_return
        train_episode_return_100=train_episode_return_100+train_episode_return
        ...
        if episode > 0 and episode % args.eval_per_train == 0:
            ...
            if args.tensorboard:
                writer.add_scalar('Train/Episode100Returns', train_episode_return_100/args.eval_per_train, episode)
                train_episode_return_100=0
            eval_sum_returns = 0.
            eval_num_episodes = 0
            ...
            for _ in range(args.evaluation_eps):
                # Run one episode
                eval_step_length, eval_episode_return = agent.run(args.max_step,writer)
